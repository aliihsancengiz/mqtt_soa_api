...
auto& namedBus = event_bus::EventBus::getInstance();
...
TEST(EventBus, canUseMemberFunctionAsEventHandler)
{
    ...
    namedBus.registerEvent<TestEvent>(handler.testTopic,
                                      std::bind(&TestEventHandler::onTestEvent, &handler,
...
TEST(EventBus, canEventStoreState)
{
    ...
    auto onTestEventWithData = [&valueFromHandler, &testTopic](std::string topic,
                                                               TestEventWithData ev) {
        EXPECT_TRUE(topic == testTopic);
...
TEST(EventBus, HandlerWithMultipleArgs)
{
    ...
    auto handlerWithArgs = [&intValueFromHandler, &boolValueFromHandler, &testTopic](
                             std::string topic, TestEvent ev, int integer_arg, bool boolean_arg) {
        EXPECT_TRUE(topic == testTopic);
